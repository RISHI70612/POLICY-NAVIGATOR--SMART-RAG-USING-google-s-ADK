...
def main():
    ...
    if not validate_api_key():
        print("\n[FAIL] GOOGLE_API_KEY not set. Please configure your API key.")
    ...
    try:
        ...
        for policy_file in policy_files:
            ...
            if not store_id:
                print(f"\n  [FAIL] No store configured for {policy_name}")
                continue
        ...
        try:
            ...
            print(f"  [FAIL] Failed to list stores: {str(e)}")

        # Summary
        print("\n\n" + "=" * 70)
        print("Demo Complete")
        print("=" * 70)
        print(f"\n[OK] Successfully uploaded {uploaded_count}/{len(policy_files)} policies")
        print("\nNext steps:")
        ...
        print(f"\n[FAIL] Demo failed with error: {str(e)}")
        import traceback
